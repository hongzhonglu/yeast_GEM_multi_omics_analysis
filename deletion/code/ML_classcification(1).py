...
def svm_class(ml_data, lable):
    ...
    pre_train = clf.predict_proba(train_data)
    pre_test = clf.predict_proba(test_data)
    ...
    return train_acc, test_acc, all_acc, recall, fscore, pre, true_data, recall_t, fscore_t
...
def roc(true_data, pre, n, method, reduction, datatype):
    if n == 50:
        n_class = 9
    elif n == 30:
        n_class = 13
        ...
        n_class = 5
    y_one_hot = label_binarize(true_data, np.arange(n_class))  # 装换成类似二进制的编码
    ...
    plt.tight_layout()
    plt.savefig('../output/ROC and AUC of {} {} {} {}.png'.format(method, n, reduction, datatype),
                dpi=600,
                transparent=True)
    plt.show()
...
def run_ml(ml_data, lable, n, components, reduction=False):
    svmtrain_acc, svmtest_acc, svmall_acc, svmrecall, svmfscore, svmpre, svmtrue_data, svmrecall_t, svmfscore_t = svm_class(ml_data, lable)
    logging.info('svm finished')
    MLPtrain_acc, MLPtest_acc, MLPall_acc, MLPrecall, MLPfscore, MLPpre, MLPtrue_data, MLPrecall_t, MLPfscore_t = MLP_class(ml_data, lable)
    logging.info('MLP finished')
    bayestrain_acc, bayestest_acc, bayesall_acc, bayesrecall, bayesfscore, bayespre, bayestrue_data, bayesrecall_t, bayesfscore_t = bayes_class(ml_data, lable)
    logging.info('bayes finished')
    ranftrain_acc, ranftest_acc, ranfall_acc, ranfrecall, ranffscore, ranfpre, ranftrue_data, ranfrecall_t, ranffscore_t = randomf_class(ml_data, lable)
    logging.info('randf finished')
    knntrain_acc, knntest_acc, knnall_acc, knnrecall, knnfscore, knnpre, knntrue_data, knnrecall_t, knnfscore_t = knn_class(ml_data, lable)
    logging.info('knn finished')
    treetrain_acc, treetest_acc, treeall_acc, treerecall, treefscore, treepre, treetrue_data, treerecall_t, treefscore_t = tree_class(ml_data, lable)
    ...
    logistictrain_acc, logistictest_acc, logisticall_acc, logisticrecall, logisfscore, logispre, logistrue_data, logisrecall_t, logisfscore_t = logistic_class(ml_data, lable)
    ...
    output = pd.DataFrame(index=['svm_{}'.format(n), 'MLP_{}'.format(n), 'bayes_{}'.format(n),
                          ...
                          columns=['train acc', 'test acc', 'all acc', 'recall', 'fscore', 'pre', 'true_data', 'recall_t', 'fscore_t', 'reduced'])
    if reduction:
        output.loc['svm_{}'.format(n), :] = [svmtrain_acc, svmtest_acc, svmall_acc, svmrecall, svmfscore, svmpre, svmtrue_data, svmrecall_t, svmfscore_t, 'kpca_{}'.format(components)]
        output.loc['MLP_{}'.format(n), :] = [MLPtrain_acc, MLPtest_acc, MLPall_acc, MLPrecall, MLPfscore, MLPpre, MLPtrue_data, MLPrecall_t, MLPfscore_t, 'kpca_{}'.format(components)]
        output.loc['bayes_{}'.format(n), :] = [bayestrain_acc, bayestest_acc, bayesall_acc, bayesrecall, bayesfscore, bayespre, bayestrue_data, bayesrecall_t, bayesfscore_t, 'kpca_{}'.format(components)]
        output.loc['randomforest_{}'.format(n), :] = [ranftrain_acc, ranftest_acc, ranfall_acc, ranfrecall, ranffscore, ranfpre, ranftrue_data, ranfrecall_t, ranffscore_t, 'kpca_{}'.format(components)]
        output.loc['knn_{}'.format(n), :] = [knntrain_acc, knntest_acc, knnall_acc, knnrecall, knnfscore, knnpre, knntrue_data, knnrecall_t, knnfscore_t, 'kpca_{}'.format(components)]
        output.loc['decisiontree_{}'.format(n), :] = [treetrain_acc, treetest_acc, treeall_acc, treerecall, treefscore, treepre, treetrue_data, treerecall_t, treefscore_t, 'kpca_{}'.format(components)]
        output.loc['logistic_{}'.format(n), :] = [logistictrain_acc, logistictest_acc, logisticall_acc, logisticrecall, logisfscore, logispre, logistrue_data, logisrecall_t, logisfscore_t, 'kpca_{}'.format(components)]

    else:
        output.loc['svm_{}'.format(n), :] = [svmtrain_acc, svmtest_acc, svmall_acc, svmrecall, svmfscore, svmpre, svmtrue_data, svmrecall_t, svmfscore_t, 'off']
        output.loc['MLP_{}'.format(n), :] = [MLPtrain_acc, MLPtest_acc, MLPall_acc, MLPrecall, MLPfscore, MLPpre, MLPtrue_data, MLPrecall_t, MLPfscore_t, 'off']
        output.loc['bayes_{}'.format(n), :] = [bayestrain_acc, bayestest_acc, bayesall_acc, bayesrecall, bayesfscore, bayespre, bayestrue_data, bayesrecall_t, bayesfscore_t, 'off']
        output.loc['randomforest_{}'.format(n), :] = [ranftrain_acc, ranftest_acc, ranfall_acc, ranfrecall, ranffscore, ranfpre, ranftrue_data, ranfrecall_t, ranffscore_t, 'off']
        output.loc['knn_{}'.format(n), :] = [knntrain_acc, knntest_acc, knnall_acc, knnrecall, knnfscore, knnpre, knntrue_data, knnrecall_t, knnfscore_t, 'off']
        output.loc['decisiontree_{}'.format(n), :] = [treetrain_acc, treetest_acc, treeall_acc, treerecall, treefscore, treepre, treetrue_data, treerecall_t, treefscore_t, 'off']
        output.loc['logistic_{}'.format(n), :] = [logistictrain_acc, logistictest_acc, logisticall_acc, logisticrecall, logisfscore, logispre, logistrue_data, logisrecall_t, logisfscore_t, 'off']
    return output
...
def get_result(n, components=0):
    ...
    if components:
        ...
        for m in only_trans.index.tolist():
            roc(only_trans.loc[m, 'true_data'], only_trans.loc[m, 'pre'], n, m, only_trans.loc[m, 'reduced'], 'only_trans')
        ...
        for m in only_flux.index.tolist():
            roc(only_flux.loc[m, 'true_data'], only_flux.loc[m, 'pre'], n, m, only_flux.loc[m, 'reduced'], 'only_flux')
        logging.info('flux finished')
        # flux and trans
        ft = kpca_reduce(ft, components)
        trans_flux = run_ml(ft, lable, n, components, reduction=True)
        for m in only_flux.index.tolist():
            roc(trans_flux.loc[m, 'true_data'], trans_flux.loc[m, 'pre'], n, m, trans_flux.loc[m, 'reduced'], 'trans_flux')
...
if __name__ == '__main__':
    ...
    writer = pd.ExcelWriter('../output/ML_result.xlsx')
