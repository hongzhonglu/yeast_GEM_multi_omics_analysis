function [v_out, f_out, LB, UB] = evaluate_objective(gammaVal, x, fbarecon, genes, reaction_expression, pos_genes_in_react_expr, idxs_genes_sorted_by_length, lowbound, deletedGeneIndex)
...
% [fbarecon,~,~]	=  prepareFastGapFill(fbarecon);
% pFBA
[solution] = optimizeCbModel(fbarecon, 'max');% use FBA instead of pFBA
f_out = solution.f;
try
    v_out = solution.v;
catch
