...
def getModelGene():
    model = read_sbml_model('../deletion/data/yeast-GEM.xml')
    geneclass = pd.read_excel(r"../deletion/data/geneclass.xlsx")
    modelgenename = [i.id for i in model.genes]
    sysname = []
    for i in range(1484):
        if str(geneclass.loc[i, 'orf name']) in modelgenename:
            sysname.append(geneclass.loc[i, 'gene'])
    return sysname
...
sysname = getModelGene()
simu_exp3 = pd.DataFrame(index=sysname,
                        columns=['exp', 'simu'])
for g in sysname:
    try:
        simu_exp3.loc[g, 'simu'] = simu1.loc[g]
        simu_exp3.loc[g, 'exp'] = exp1.loc[g, 'log2relT']
    except KeyError:
        pass
simu_exp3.dropna(inplace=True)
# double
...
plt.scatter(simu_exp2.loc[:, 'simu'],
            ...
            marker='s',
            alpha=0.5)
# plt.scatter(simu_exp3.loc[:, 'simu'],
#             simu_exp3['exp'],
#             c='#C44746',
#             marker='d',
#             alpha=0.8)
...
pccs = np.corrcoef(np.array(simu_exp3.loc[:, 'simu'].values.tolist()),
                   np.array(simu_exp3['exp'].values.tolist()))
pairp = stats.ttest_rel(np.array(simu_exp3.loc[:, 'simu'].values.tolist()),
                        np.array(simu_exp3['exp'].values.tolist())
...
plt.tight_layout()
plt.savefig("../deletion/output/growth_exp_simu_in_model.jpg", dpi=600)
plt.show()
