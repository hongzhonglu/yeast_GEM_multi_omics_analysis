...
exp = pd.read_table('../data/SGA_NxN_clustered.tsv')
exp.index = exp.loc[:, 'A']
lie = exp.index.values.tolist()
hang = exp.columns.values.tolist()
modelgene = [g.id for g in model.genes]

pair1 = [p1 for p1 in lie if p1 in modelgene]
pair2 = [p2 for p2 in hang if p2 in modelgene]

chongfu = [p for p in pair1 if p in pair2]

print('the double deletion process may take 24 hours')
