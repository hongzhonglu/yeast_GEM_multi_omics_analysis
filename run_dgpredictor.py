...
if __name__ == '__main__':
    ...
    # rxns = pd.read_excel('./dgpredictor.xlsx')
    pH = 7.0
    I = 0.1
    db_smiles = load_smiles()
    ...
    # for i in range(1000):
    #     print(i/4063)
    #     rxn_str = rxns.loc[i, 'eq']
    #     if str(rxn_str) != 'nan':
    #         try:
    #             rxn_dict = parse_formula(rxn_str)
    #             mu, std, rule_df1, rule_df2 = get_dG0(
    #                 rxn_dict, 'R00801', pH, I, loaded_model, molsig_r1, molsig_r2, novel_decomposed_r1=None, novel_decomposed_r2=None,
    #                 novel_mets=None)
    #             print('{}: {} kj/mol'.format(rxn_str, mu))
    #             rxns.loc[i, 'detaG'] = mu
    #         except KeyError as e:
    #             print('KeyError arised:{} in {}'.format(e, rxn_str))
    #             rxns.loc[i, 'detaG'] = 'KeyError arised:{}'.format(e)
    # rxns.to_excel('./detaG1000.xlsx')
    #
    # rxns = pd.read_excel('./dgpredictor.xlsx')
    ...
    #             print('KeyError arised:{} in {}'.format(e, rxn_str))
    #             rxns.loc[i, 'detaG'] = 'KeyError arised:{}'.format(e)
    # rxns.to_excel('./detaG3000.xlsx')

    rxns = pd.read_excel('./dgpredictor.xlsx')
    for i in range(len(rxns.index)):
        print(i / 4063)
        rxn_str = rxns.loc[i, 'eq']
        if str(rxn_str) != 'nan':
            try:
